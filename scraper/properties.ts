...
import { BASE_URL, fetchPageData, sleep } from './util.ts';
...
async function parseSalesData(data: HTMLDocument) {
	const sales: Sale[] = [];
	const table = Array.from(
		data.getElementsByTagName('table'),
	).filter((b) =>
	...
	for (let i = 1; i < rows.length; i++) {
		...
		sales.push({
			...
			price: +row[1]!
				.querySelector('div')!
				.innerHTML.replace(/(?:&nbsp;|\$|,)/g, ''),
			purchaseCode: row[4]!.querySelector('a')!.innerHTML.replace(
				/&nbsp;/g,
				'',
			),
		});
...
async function parseAssessmentData(data: HTMLDocument) {
